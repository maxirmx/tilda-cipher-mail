...
DIR0=$( dirname "$0" )
SERVICENAME="tilda-cipher-mail-2"
...
echo -n "$SERVICENAME daemon started on ">>"$LOGFILE"
date>>"$LOGFILE"
...
if [[ -f "$DATAFILE" ]]; then
  ...
  "$DIR0"/convert.php "$JSONFILE" "$CSVFILE"
else
...
echo -n "$SERVICENAME daemon finished on ">>"$LOGFILE"
date>>"$LOGFILE"
